...
function tick() {
  ...
  var dx = state.direction.x;
  var dy = state.direction.y;
  var highestIndex = state.snake.length - 1;
  var tail = {};
  var interval = TICK;
  Object.assign(tail, state.snake[state.snake.length - 1]);
  var didScore = head.x === state.prey.x && head.y === state.prey.y;

  if (state.runState === STATE_RUNNING) {
    for (var idx = highestIndex; idx > -1; idx--) {
      var sq = state.snake[idx];
...
function draw() {
  ...
  for (var idx = 0; idx < state.snake.length; idx++) {
    var _state$snake$idx = state.snake[idx],
        _x = _state$snake$idx.x,
        _y = _state$snake$idx.y;
    drawPixel(state.paint_color, _x, _y);
  }

  var _state$prey = state.prey,
      x = _state$prey.x,
      y = _state$prey.y;
...
window.onload = function () {
  ...
  window.onkeydown = function (e) {
    var direction = DIRECTIONS_MAP[e.keyCode];
    var snake_color = COLORS[e.keyCode];
